...
export class NodeInstaller extends BaseInstaller {
  ...
  async postInstall() {
    // Update shell configuration files based on user selection
    const shellConfigs = this.getShellConfigFiles();
    const homeDir = os.homedir();
    const version = getCurrentVersion();

    const nvmConfig = `export NVM_DIR="$HOME/.nvm"
...
nvm use default --silent 2>/dev/null || true`;
    ...
    for (const config of shellConfigs) {
      ...
      try {
        await updateModification(configPath, 'nvm-config', nvmConfig, version);
        this.logger.debug(`Updated ${config} with NVM configuration`);
      } catch (_error) {
        this.logger.debug(`Could not update ${config}: ${_error.message}`);
      }
    }

    // Always update .profile for login shells
    const profilePath = path.join(homeDir, '.profile');
    try {
      await updateModification(profilePath, 'nvm-config', nvmConfig, version);
      this.logger.debug('Updated .profile with NVM configuration');
    } catch (_error) {
      this.logger.debug(`Could not update .profile: ${_error.message}`);
    }

    const shellName = this.globalConfig?.shellConfig || 'shell configuration';
    this.logger.info(`NVM has been added to your ${shellName}.`);

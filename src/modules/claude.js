...
export class ClaudeInstaller extends BaseInstaller {
  ...
  async configureShell() {
    ...
    const nvmDefaultContent = `# Ensure NVM default version is loaded
[ -s "$NVM_DIR/nvm.sh" ] && nvm use default >/dev/null 2>&1`;
    ...
    for (const shellConfig of shellConfigs) {
      ...
      try {
        await updateModification(configPath, 'claude-nvm-default', nvmDefaultContent, version);
        this.logger.debug(`Added NVM default loading to ${shellConfig}`);
      } catch (_error) {
